...
public class Neuriteness2D{
	...
	private void computeNeuritenessWithPhase(){
		Matrix hessian = new Matrix(2,2);
		...
		for(int y=0;y<source.getHeight();y++){
			for(int x=0;x<source.getWidth();x++){
				...
				{
					//Vertical approximation
					double a = (raster.getSample(x, y+1, 0)*phaseCong[x][y+1]+raster.getSample(x, y-1, 0)*phaseCong[x][y-1]-2*phaseCong[x][y]*raster.getSample(x, y, 0));
						hessian.set(1, 1, a);
				} else hessian.set(1, 1, 0);
				if(x+1<source.getWidth() && x-1>=0 && y+1 < source.getHeight() && y-1>=0)
				{
